...
class Solver:
    def __init__(
        self, init_state, goal_state, heuristic_func="manhattan", max_iter=2500
    ):
    ...
    def solve_a_star(self):
        ...
        init_node = GameState(
            self.__init_state.flatten().tolist(),
            self.__goal_state.flatten().tolist(),
            level,
            parent=None,
            heuristic_func=self.__heuristic_func,
        )
        nodes.put(init_node)
        ...
        while nodes.qsize() and epochs <= self.__max_iter:
            ...
            if cur_state == self.__goal_state.flatten().tolist():
                self.__summary = str(
                    "A* took "
                    + str(cur_node.get_level())
                    + " steps to get from initial state to the desired goal, visited total of "
                    + str(epochs)
                    + " nodes, and took around "
                    + str(np.round(time.perf_counter() - start_time, 4))
                    + " seconds to reach the desired solution."
                )
            ...
            i, j = (
                empty_tile // self.__goal_state.shape[0],
                empty_tile % self.__goal_state.shape[0],
            )

            cur_state = np.array(cur_state).reshape(
                self.__goal_state.shape[0], self.__goal_state.shape[0]
            )
            for x, y in zip(x_axis, y_axis):
                new_state = np.array(cur_state)
                if (
                    i + x >= 0
                    and i + x < self.__goal_state.shape[0]
                    and j + y >= 0
                    and j + y < self.__goal_state.shape[0]
                ):
